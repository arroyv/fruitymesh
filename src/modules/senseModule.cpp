...
void senseModule::TimerEventHandler(u16 passedTimeDs)
{
    ...
    logt("ERROR", "Boardconfig->batteryAdcInputPin: %u", (u32)Boardconfig->batteryAdcInputPin);
    logt("ERROR", "SHOULD_IV_TRIGGER: %u", (u32)SHOULD_IV_TRIGGER(GS->appTimerDs, passedTimeDs, batteryMeasurementIntervalDs));
    logt("ERROR", "passedTimeDs: %u", (u32)passedTimeDs);
    logt("ERROR", "batteryMeasurementIntervalDs: %u", (u32)batteryMeasurementIntervalDs);
    ...
    if( (GS->appTimerDs < SEC_TO_DS(40) && Boardconfig->batteryAdcInputPin != -1 )
        || SHOULD_IV_TRIGGER(GS->appTimerDs, passedTimeDs, batteryMeasurementIntervalDs)){
            logt("ERROR", "In side TimerEventHandler and right before the call to the BatteryVoltageADC function ");
        BatteryVoltageADC();
    }
...
void senseModule::SendStatus(NodeId toNode, u8 requestHandle, MessageType messageType) const
{
    ...
    SendModuleActionMessage(
        messageType,
        toNode,
        (u8)senseModuleActionResponseMessages::STATUS,
        requestHandle,
        (u8*)&data,
...
// void senseModule::SendDeviceInfoV2(NodeId toNode, u8 requestHandle, MessageType messageType) const
// {
//     senseModuleDeviceInfoV2Message data;
...
//         messageType,
//         toNode,
//         (u8)senseModuleActionResponseMessages::DEVICE_INFO_V2,
//         requestHandle,
//         (u8*)&data,
//         SIZEOF_SENSE_MODULE_DEVICE_INFO_V2_MESSAGE,
//         false
//     );
// }

// void senseModule::SendNearbyNodes(NodeId toNode, u8 requestHandle, MessageType messageType)
// {
...
//         messageType,
//         toNode,
//         (u8)senseModuleActionResponseMessages::NEARBY_NODES,
//         requestHandle,
//         buffer,
//         packetSize,
//         false
...
// void senseModule::SendAllConnections(NodeId toNode, u8 requestHandle, MessageType messageType) const
// {
//     senseModuleConnectionsMessage message;
//     CheckedMemset(&message, 0x00, sizeof(senseModuleConnectionsMessage));
...
//         NODE_ID_BROADCAST,
//         (u8)senseModuleActionResponseMessages::ALL_CONNECTIONS,
//         requestHandle,
//         (u8*)&message,
//         SIZEOF_SENSE_MODULE_CONNECTIONS_MESSAGE,
//         false
//     );
// }

// void senseModule::SendAllConnectionsVerbose(NodeId toNode, u8 requestHandle, u32 connectionIndex) const
// {
...
//             senseModuleConnectionsVerboseMessage message;
...
//                 MessageType::MODULE_ACTION_RESPONSE,
//                 toNode,
//                 (u8)senseModuleActionResponseMessages::ALL_CONNECTIONS_VERBOSE,
//                 requestHandle,
//                 (u8*)&message,
//                 sizeof(message),
//                 false
...
// void senseModule::SendRebootReason(NodeId toNode, u8 requestHandle) const
// {
//     SendModuleActionMessage(
//         MessageType::MODULE_ACTION_RESPONSE,
//         toNode,
//         (u8)senseModuleActionResponseMessages::REBOOT_REASON,
//         requestHandle,
...
//     );
// }
void senseModule::SendErrors(NodeId toNode, u8 requestHandle) const{
    ...
    senseModuleErrorLogEntryMessage data;
    for(int i=0; i< GS->logger.errorLogPosition; i++){
        ...
        SendModuleActionMessage(
            MessageType::MODULE_ACTION_RESPONSE,
            toNode,
            (u8)senseModuleActionResponseMessages::ERROR_LOG_ENTRY,
            requestHandle,
            (u8*)&data,
            SIZEOF_SENSE_MODULE_ERROR_LOG_ENTRY_MESSAGE,
            false
        );
    }
...
// void senseModule::SendLiveReport(LiveReportTypes type, u16 requestHandle, u32 extra, u32 extra2) const
// {
//     //Live reporting states are off=0, error=50, warn=100, info=150, debug=200
//     if (type > configuration.liveReportingState) return;
//     senseModuleLiveReportMessage data;
...
//         (u8)senseModuleGeneralMessages::LIVE_REPORT,
//         requestHandle,
//         (u8*)&data,
//         SIZEOF_SENSE_MODULE_LIVE_REPORT_MESSAGE,
//         false
//     );
// }

// void senseModule::StartConnectionRSSIMeasurement(MeshConnection& connection) const{
...
// void senseModule::GapAdvertisementReportEventHandler(const FruityHal::GapAdvertisementReportEvent & advertisementReportEvent)
// {
        ...
        if(TERMARGS(0, "action"))
        {
            ...
            if(TERMARGS(3, "get_status"))
            {
                SendModuleActionMessage(
                    ...
                    destinationNode,
                    (u8)senseModuleTriggerActionMessages::GET_STATUS,
                    0,
                    nullptr,
                    0,
                    false
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::GET_DEVICE_INFO_V2,
            //         0,
            //         nullptr,
            //         0,
            //         false
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::GET_ALL_CONNECTIONS,
            //         0,
            //         nullptr,
            //         0,
            //         false
            ...
            // {
            //     senseModuleConnectionsVerboseRequestMessage message;
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::GET_ALL_CONNECTIONS_VERBOSE,
            //         0,
            //         (u8*)&message,
            //         sizeof(message),
            //         false
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::GET_NEARBY_NODES,
            //         0,
            //         nullptr,
            //         0,
            //         false
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::SET_INITIALIZED,
            //         0,
            //         nullptr,
            //         0,
            //         false
            ...
            //     destinationNode = SENSE_MODULE_MAX_HOPS;
            //     senseModuleKeepAliveMessage msg;
            //     CheckedMemset(&msg, 0, sizeof(msg));
            //     msg.fromSink = GET_DEVICE_TYPE() == DeviceType::SINK;
            //     SendModuleActionMessage(
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::SET_KEEP_ALIVE,
            //         0,
            //         (u8*)&msg,
            //         sizeof(msg),
            //         false
            ...
            else if(TERMARGS(3, "get_errors"))
            {
                SendModuleActionMessage(
                    MessageType::MODULE_TRIGGER_ACTION,
                    destinationNode,
                    (u8)senseModuleTriggerActionMessages::GET_ERRORS,
                    0,
                    nullptr,
                    0,
                    false
            ...
            //             MessageType::MODULE_TRIGGER_ACTION,
            //             destinationNode,
            //             (u8)senseModuleTriggerActionMessages::SET_LIVEREPORTING,
            //             0,
            //             &liveReportingState,
            //             1,
            //             false
            ...
            //         MessageType::MODULE_TRIGGER_ACTION,
            //         destinationNode,
            //         (u8)senseModuleTriggerActionMessages::GET_REBOOT_REASON,
            //         0,
            //         nullptr,
            //         0,
            //         false
...
void senseModule::MeshMessageReceivedHandler(BaseConnection* connection, BaseConnectionSendData* sendData, ConnPacketHeader const * packetHeader)
{
    ...
    if(packetHeader->messageType == MessageType::MODULE_TRIGGER_ACTION){
        ...
        if(packet->moduleId == vendorModuleId){
            ...
            if(actionType == senseModuleTriggerActionMessages::GET_STATUS)
            {
            ...
            // else if(actionType == senseModuleTriggerActionMessages::GET_DEVICE_INFO_V2)
            // {
            ...
            // else if(actionType == senseModuleTriggerActionMessages::GET_ALL_CONNECTIONS)
            // {
            //     senseModule::SendAllConnections(packetHeader->sender, packet->requestHandle, MessageType::MODULE_ACTION_RESPONSE);
            // }
            // //We were queried for our connections with verbosity
            // else if (actionType == senseModuleTriggerActionMessages::GET_ALL_CONNECTIONS_VERBOSE && sendData->dataLength >= sizeof(senseModuleConnectionsVerboseRequestMessage))
            // {
            //     const senseModuleConnectionsVerboseRequestMessage* message = (const senseModuleConnectionsVerboseRequestMessage*)packet->data;
            //     senseModule::SendAllConnectionsVerbose(packetHeader->sender, packet->requestHandle, message->connectionIndex);
            // }
            // //We were queried for nearby nodes (nodes in the join_me buffer)
            // else if(actionType == senseModuleTriggerActionMessages::GET_NEARBY_NODES)
            // {
            //     senseModule::SendNearbyNodes(packetHeader->sender, packet->requestHandle, MessageType::MODULE_ACTION_RESPONSE);
            // }
            // //We should set ourselves initialized
            // else if(actionType == senseModuleTriggerActionMessages::SET_INITIALIZED)
            // {
            //     GS->node.initializedByGateway = true;
            //     SendModuleActionMessage(
            //         MessageType::MODULE_ACTION_RESPONSE,
            //         packet->header.sender,
            //         (u8)senseModuleActionResponseMessages::SET_INITIALIZED_RESULT,
            ...
            //     );
            // }
            // else if(actionType == senseModuleTriggerActionMessages::SET_KEEP_ALIVE)
            // {
            ...
            //     else
            //     {
            //         const senseModuleKeepAliveMessage* msg = (const senseModuleKeepAliveMessage*)&packet->data;
            //         comesFromSink = msg->fromSink;
            //     }
            //     if (connection != nullptr && comesFromSink)
            //     {
            //         u16 receivedHopsToSink = SENSE_MODULE_MAX_HOPS - packetHeader->receiver + 1;
            ...
            else if(actionType == senseModuleTriggerActionMessages::GET_ERRORS)
            {
                SendErrors(packet->header.sender, packet->requestHandle);
            }
            // //Configures livereporting
            // else if(actionType == senseModuleTriggerActionMessages::SET_LIVEREPORTING)
            // {
            ...
            // else if(actionType == senseModuleTriggerActionMessages::GET_REBOOT_REASON)
            // {
    ...
    if(packetHeader->messageType == MessageType::MODULE_ACTION_RESPONSE){
        ...
        if(packet->moduleId == vendorModuleId)
        {
            ...
            // if(actionType == senseModuleActionResponseMessages::ALL_CONNECTIONS)
            // {
            //     senseModuleConnectionsMessage const * packetData = (senseModuleConnectionsMessage const *) (packet->data);
            //     logjson("STATUSMOD", "{\"type\":\"connections\",\"nodeId\":%d,\"module\":%u,\"partners\":[%d,%d,%d,%d],\"rssiValues\":[%d,%d,%d,%d]}" SEP, packet->header.sender, (u8)ModuleId::SENSE_MODULE, packetData->partner1, packetData->partner2, packetData->partner3, packetData->partner4, packetData->rssi1, packetData->rssi2, packetData->rssi3, packetData->rssi4);
            // }
            // if (actionType == senseModuleActionResponseMessages::ALL_CONNECTIONS_VERBOSE)
            // {
            //     senseModuleConnectionsVerboseMessage const* packetData = (senseModuleConnectionsVerboseMessage const*)(packet->data);
            //     bool unknownButFittingVersion = false;
            //     if (packetData->header.version > senseModuleConnectionsVerboseHeader::MAX_KNOWN_VERSION && sendData->dataLength >= senseModuleConnectionsVerboseMessage::SIZEOF_MAX_KNOWN_VERSION + SIZEOF_CONN_PACKET_MODULE)
            //     {
            //         unknownButFittingVersion = true;
            //     }
            //     if (packetData->header.version == senseModuleConnectionsVerboseHeader::MAX_KNOWN_VERSION || unknownButFittingVersion)
            //     {
            ...
            //         logjson_partial("STATUSMOD", "\"module\":%u,"                                 , (u32)ModuleId::SENSE_MODULE);
            ...
            //     }
            // }
            // else if(actionType == senseModuleActionResponseMessages::DEVICE_INFO_V2)
            // {
            //     //Print packet to console
            //     senseModuleDeviceInfoV2Message const * data = (senseModuleDeviceInfoV2Message const *) (packet->data);
            ...
            //     logjson_partial("STATUSMOD", "{\"nodeId\":%u,\"type\":\"device_info\",\"module\":%u,", packet->header.sender, (u8)ModuleId::SENSE_MODULE);
            ...
            // }
           /* else*/ 
           if(actionType == senseModuleActionResponseMessages::STATUS)
            {
                //Print packet to console
                ...
                logjson_partial("STATUSMOD", "\"batteryInfo\":%u,", data->batteryInfo);
                logjson("STATUSMOD", "}" SEP);
            }
            // else if(actionType == senseModuleActionResponseMessages::NEARBY_NODES)
            // {
            //     //Print packet to console
            //     logjson_partial("STATUSMOD", "{\"nodeId\":%u,\"type\":\"nearby_nodes\",\"module\":%u,\"nodes\":[", packet->header.sender, (u8)ModuleId::SENSE_MODULE);
            ...
            //     logjson("STATUSMOD", "]}" SEP);
            // }
            // else if(actionType == senseModuleActionResponseMessages::SET_INITIALIZED_RESULT)
            // {
            //     logjson("STATUSMOD", "{\"type\":\"set_init_result\",\"nodeId\":%u,\"module\":%u}" SEP, packet->header.sender, (u8)ModuleId::SENSE_MODULE);
            // }
            else if(actionType == senseModuleActionResponseMessages::ERROR_LOG_ENTRY)
            {
                senseModuleErrorLogEntryMessage const * data = (senseModuleErrorLogEntryMessage const *) (packet->data);

                logjson_partial("STATUSMOD", "{\"type\":\"error_log_entry\",\"nodeId\":%u,\"module\":%u,", packet->header.sender, (u32) SENSE_MODULE_ID);
            ...
            // else if(actionType == senseModuleActionResponseMessages::REBOOT_REASON)
            // {
            //     RamRetainStruct const * data = (RamRetainStruct const *) (packet->data);
            //     logjson_partial("STATUSMOD", "{\"type\":\"reboot_reason\",\"nodeId\":%u,\"module\":%u,", packet->header.sender, (u8)ModuleId::SENSE_MODULE);
...
// void senseModule::MeshConnectionChangedHandler(MeshConnection& connection)
// {
...
void senseModule::InitBatteryVoltageADC() {
    ...
    {
        logt("ERROR", "  Boardconfig->batteryAdcInputPin == -2");
        // Battery input
        pin = 0xFF;
    }
    ...
    {
        logt("ERROR", "   AdcReference::ADC_REFERENCE_1_4_POWER_SUPPLY, AdcResoultion::ADC_10_BIT, AdcGain::ADC_GAIN_1_5");
        err = FruityHal::AdcConfigureChannel(pin,
                                       FruityHal::AdcReference::ADC_REFERENCE_0_6V, 
                                       FruityHal::AdcResoultion::ADC_10_BIT, 
                                       FruityHal::AdcGain::ADC_GAIN_1_6);
...
void senseModule::BatteryVoltageADC(){
...
void senseModule::ConvertADCtoVoltage()
{
    logt("ERROR", "In side ConvertADCtoVoltage");
#if IS_ACTIVE(BATTERY_MEASUREMENT)
    logt("ERROR", "ACTIVE_BATTERY_MEASUREMENT");
    u32 adc_sum_value = 0;
    logt("ERROR", "BATTERY_SAMPLES_IN_BUFFER: %u", (u32)BATTERY_SAMPLES_IN_BUFFER);
...
#if FEATURE_AVAILABLE(ADC_INTERNAL_MEASUREMENT)
    logt("ERROR", "ADC_INTERNAL_MEASUREMENT_AVAILABLE");
    logt("ERROR", "Boardconfig->batteryAdcInputPin >= 0?: %u", (u32)Boardconfig->batteryAdcInputPin);
    logt("ERROR", "Boardconfig->voltageDividerR1 > 0?: %u", (u32)Boardconfig->voltageDividerR1);
    logt("ERROR", "Boardconfig->voltageDividerR2 > 0?: %u", (u32)Boardconfig->voltageDividerR2);
    if (Boardconfig->batteryAdcInputPin >= 0 && Boardconfig->voltageDividerR1,Boardconfig->voltageDividerR2 > 0){
        logt("ERROR", "External Voltage divider avalible");
        u16 voltageDividerDv = ExternalVoltageDividerDv(Boardconfig->voltageDividerR1, Boardconfig->voltageDividerR2);
        logt("ERROR", "voltageDividerDv: %u", (u32)voltageDividerDv);
        batteryVoltageDv = FruityHal::AdcConvertSampleToDeciVoltage(adc_sum_value / BATTERY_SAMPLES_IN_BUFFER, voltageDividerDv);
        logt("ERROR", "batteryVoltageDv update: %u", (u32)batteryVoltageDv);
    }
    else {
...
u8 senseModule::GetBatteryVoltage() const
{
    logt("ERROR", "In side GetBatteryVoltage");
    logt("ERROR", "batteryVoltageDv: %u", (u32)batteryVoltageDv);
    return batteryVoltageDv;
}

u16 senseModule::ExternalVoltageDividerDv(u32 Resistor1, u32 Resistor2)
{
...
MeshAccessAuthorization senseModule::CheckMeshAccessPacketAuthorization(BaseConnectionSendData * sendData, u8 const * data, FmKeyId fmKeyId, DataDirection direction)
{
        ...
        || packet->messageType == MessageType::MODULE_ACTION_RESPONSE)
    {
        ...
        if (mod->moduleId == vendorModuleId)
        {
            ...
            {
                static_assert((u8)senseModuleTriggerActionMessages::GET_STATUS         == (u8)senseModuleActionResponseMessages::STATUS,         "The following check assumes that both have the same value in both directions");
                // static_assert((u8)senseModuleTriggerActionMessages::GET_DEVICE_INFO_V2 == (u8)senseModuleActionResponseMessages::DEVICE_INFO_V2, "The following check assumes that both have the same value in both directions");
                if (mod->actionType == (u8)senseModuleTriggerActionMessages::GET_STATUS
                    /*|| mod->actionType == (u8)senseModuleTriggerActionMessages::GET_DEVICE_INFO_V2*/)
                {
